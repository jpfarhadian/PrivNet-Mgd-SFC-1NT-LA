...
#JP Setting the variables
$location = 'westeurope'
$customerName = 'ban'
$adminGrp = ''
$environ = 'tst'
$mgmtAkv = ''
$clusterAdminUsername = 'bandana'
...
if ($vnetACLs.ObjectId -notcontains $sfcSPN.Id)
{
    New-AzRoleAssignment -ObjectId $sfcSPN.Id -RoleDefinitionId "4d97b98b-1d4f-4787-a291-c67834d212e7" -Scope $vnet.Id
...
{
    $akvDeploy = (Get-AzKeyVault -VaultName $akvName -ResourceGroupName $rg)
...
$mgdSfcClusterRg = Get-AzVmss -VMScaleSetName $ntName
...
if ($sfDeploy.ProvisioningState -ne "Succeeded")
{
    Write-Host "Service Fabric Failed. Skipping Internal Load Balancer Deployment and Breaking out of Script" -ForegroundColor Red -BackgroundColor Gray
    Break
}
